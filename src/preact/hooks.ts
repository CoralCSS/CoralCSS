/**
 * CoralCSS Preact Hooks
 *
 * Custom hooks for using CoralCSS in Preact components.
 * @module preact/hooks
 */
...
/**
 * Hook for dark mode aware class names
 *
 * @example
 * ```tsx
 * function ThemeAwareBox() {
 *   const className = useDarkMode({
 *     light: 'bg-white text-gray-900',
 *     dark: 'bg-gray-900 text-white',
 *   })
 *   return <div className={className}>Theme aware content</div>
 * }
 * ```
 */
export function useDarkMode(classes: {
  light: string
  dark: string
}) {
  const { process } = useCoralContext()
