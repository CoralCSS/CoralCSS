...
const aiComponents = [
  ...
  <textarea
    ...
    rows="3"
    class="w-full px-4 py-3 pr-24 resize-none rounded-xl focus:outline-none focus:ring-2 focus:ring-coral-500"
  ></textarea>
  <div class="absolute bottom-3 right-3 flex items-center gap-2">
    <span class="text-xs text-muted-foreground/70">0/4000</span>
    <button class="p-2 bg-coral-500/100 text-white rounded-lg hover:bg-coral-600 transition-colors disabled:opacity-50">
  ...
  <p>
    ...
    <span class="inline-block w-2 h-5 bg-coral-500/100 animate-pulse ml-0.5"></span>
  </p>
</div>`,
    props: [
  ...
  <div class="absolute top-full mt-2 w-72 bg-card border border-border rounded-xl shadow-lg p-2 z-10">
    <button class="w-full flex items-center gap-3 p-3 hover:bg-muted/50 rounded-lg">
      <span class="w-2 h-2 bg-purple-500/100 rounded-full"></span>
      <div class="flex-1 text-left">
        <p class="font-medium">Claude 3 Opus</p>
        <p class="text-xs text-muted-foreground">Most capable, best for complex tasks</p>
      </div>
    </button>
    <button class="w-full flex items-center gap-3 p-3 bg-coral-500/10 rounded-lg">
      <span class="w-2 h-2 bg-green-500/100 rounded-full"></span>
      <div class="flex-1 text-left">
        <p class="font-medium text-coral-600">Claude 3.5 Sonnet</p>
        <p class="text-xs text-muted-foreground">Fast and intelligent</p>
      </div>
      <svg class="w-4 h-4 text-coral-500"><!-- check --></svg>
    </button>
  </div>
</div>`,
    props: [
  ...
  <div class="flex-1">
    ...
    <div class="h-2 bg-muted rounded-full overflow-hidden">
      <div class="h-full bg-coral-500/100 rounded-full" style="width: 30%"></div>
    </div>
  </div>
</div>`,
    props: [
  ...
  <div class="w-8 h-8 bg-gradient-to-br from-coral-400 to-coral-600 rounded-lg flex items-center justify-center text-white flex-shrink-0">
...
function greet(name: string) {
  ...
  <div class="w-8 h-8 bg-gradient-to-br from-coral-400 to-coral-600 rounded-lg flex items-center justify-center">
  ...
  <button class="p-2 text-muted-foreground/70 hover:text-green-600 hover:bg-green-500/10 rounded-lg transition-colors">
    <svg class="w-4 h-4"><!-- thumbs up --></svg>
  </button>
  <button class="p-2 text-muted-foreground/70 hover:text-red-600 hover:bg-red-500/10 rounded-lg transition-colors">
  ...
  <div class="w-8 h-8 bg-blue-500/20 text-blue-600 rounded flex items-center justify-center flex-shrink-0">
  ...
  <div class="p-4 border-b border-border">
    <button class="w-full flex items-center gap-2 px-4 py-2 bg-coral-500/100 text-white rounded-lg hover:bg-coral-600 transition-colors">
  ...
  <div class="flex items-center justify-between px-4 py-2 bg-muted/50 border-b border-border">
    ...
    <button class="text-sm text-coral-500 hover:text-coral-600">Reset</button>
  </div>
  <textarea
  ...
  <div>
    ...
    <input type="range" min="0" max="2" step="0.1" value="0.7" class="w-full accent-coral-500" />
    <div class="flex justify-between text-xs text-muted-foreground/70 mt-1">
      <span>Precise</span>
      <span>Creative</span>
    </div>
  </div>
  ...
  },
  {
    ...
    usage: `<button class="flex items-center gap-2 px-4 py-2 bg-red-500/100 text-white rounded-lg hover:bg-red-600 transition-colors">
  ...
  <div class="border rounded-xl overflow-hidden">
    <div class="px-4 py-2 bg-purple-500/10 border-b border-border flex items-center gap-2">
      <span class="w-2 h-2 bg-purple-500/100 rounded-full"></span>
      <span class="font-medium text-purple-700">GPT-4</span>
    </div>
  ...
  <div class="border rounded-xl overflow-hidden">
    <div class="px-4 py-2 bg-coral-500/10 border-b border-border flex items-center gap-2">
      <span class="w-2 h-2 bg-coral-500/100 rounded-full"></span>
      <span class="font-medium text-coral-700">Claude</span>
    </div>
    <div class="p-4 prose prose-sm">
  ...
  <div class="aspect-square bg-muted rounded-xl flex items-center justify-center">
    <div class="text-center">
      <div class="w-12 h-12 border-4 border-coral-500 border-t border-border-transparent rounded-full animate-spin mx-auto"></div>
      <p class="mt-4 text-muted-foreground">Generating image...</p>
      <p class="text-sm text-muted-foreground/70">This may take a few seconds</p>
    </div>
  </div>
  ...
  },
  {
    ...
    usage: `<div class="flex items-start gap-3 p-4 bg-red-500/10 border border-r border-red-500/30 rounded-xl">
  <svg class="w-5 h-5 text-red-500 flex-shrink-0 mt-0.5"><!-- error icon --></svg>
  <div>
    <p class="font-medium text-red-800">Rate limit exceeded</p>
    <p class="text-sm text-red-600 mt-1">Please wait a moment before trying again.</p>
    <button class="mt-2 text-sm text-red-700 hover:text-red-800 font-medium">
      Try again
    </button>
  </div>
</div>`,
  ...
  return (
    <div className="relative border border-border rounded-xl bg-card shadow-sm max-w-xl">
      <textarea
        placeholder="Ask me anything..."
        rows={3}
        className="w-full px-4 py-3 pr-24 resize-none rounded-xl focus:outline-none focus:ring-2 focus:ring-coral-500"
        defaultValue=""
      ></textarea>
      <div className="absolute bottom-3 right-3 flex items-center gap-2">
        <span className="text-xs text-muted-foreground/70">0/4000</span>
        <button className="p-2 bg-coral-500/100 text-white rounded-lg hover:bg-coral-600 transition-colors disabled:opacity-50">
  ...
  return (
    <div className="prose prose-coral max-w-xl">
      <p>
        ...
        <span className="inline-block w-2 h-5 bg-coral-500/100 animate-pulse ml-0.5 align-middle"></span>
      </p>
    </div>
  )
}
    ...
    <div className="flex gap-4">
      ...
      <div className="w-72 bg-card border border-border rounded-xl shadow-lg p-2">
        <button className="w-full flex items-center gap-3 p-3 hover:bg-muted/50 rounded-lg text-left">
          <span className="w-2 h-2 bg-purple-500/100 rounded-full"></span>
          <div className="flex-1">
            <p className="font-medium">Claude 3 Opus</p>
            <p className="text-xs text-muted-foreground">Most capable</p>
          </div>
        </button>
        <button className="w-full flex items-center gap-3 p-3 bg-coral-500/100/10 rounded-lg text-left">
          <span className="w-2 h-2 bg-green-500/100 rounded-full"></span>
          <div className="flex-1">
            <p className="font-medium text-coral-500">Claude 3.5 Sonnet</p>
    ...
    <div className="max-w-sm space-y-4">
      <div>
        ...
        <div className="h-2 bg-muted rounded-full overflow-hidden">
          <div className="h-full bg-coral-500/100 rounded-full transition-all" style={{ width: '30%' }}></div>
        </div>
      </div>
      <div>
        <div className="flex justify-between mb-1 text-sm">
          <span className="text-muted-foreground">Tokens used</span>
          <span className="font-medium text-amber-600">3,500 / 4,096</span>
        </div>
        <div className="h-2 bg-muted rounded-full overflow-hidden">
          <div className="h-full bg-amber-500/100 rounded-full transition-all" style={{ width: '85%' }}></div>
        </div>
      </div>
    </div>
  )
...
function AIMessagePreview() {
  return (
    <div className="flex gap-4 max-w-2xl">
      <div className="w-8 h-8 bg-gradient-to-br from-coral-400 to-coral-600 rounded-lg flex items-center justify-center text-white flex-shrink-0">
        <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
function ResponseActionsPreview() {
  return (
    <div className="flex items-center gap-1">
      <button className="p-2 text-muted-foreground/70 hover:text-muted-foreground hover:bg-muted rounded-lg transition-colors">
        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" />
        </svg>
      </button>
      ...
      <button className="p-2 text-muted-foreground/70 hover:text-green-600 hover:bg-green-500/10 rounded-lg transition-colors">
        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 10h4.764a2 2 0 011.789 2.894l-3.5 7A2 2 0 0115.263 21h-4.017c-.163 0-.326-.02-.485-.06L7 20m7-10V5a2 2 0 00-2-2h-.095c-.5 0-.905.405-.905.905 0 .714-.211 1.412-.608 2.006L7 11v9m7-10h-2M7 20H5a2 2 0 01-2-2v-6a2 2 0 012-2h2.5" />
        </svg>
      </button>
      <button className="p-2 text-muted-foreground/70 hover:text-red-600 hover:bg-red-500/10 rounded-lg transition-colors">
        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  return (
    <div className="flex flex-wrap gap-2">
      <div className="flex items-center gap-2 p-2 bg-muted/50 border rounded-lg">
        <div className="w-8 h-8 bg-blue-500/20 text-blue-600 rounded flex items-center justify-center flex-shrink-0">
          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="flex items-center gap-2 p-2 bg-muted/50 border rounded-lg">
        <div className="w-8 h-8 bg-green-500/20 text-green-600 rounded flex items-center justify-center flex-shrink-0">
          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  return (
    <div className="w-full max-w-sm mx-auto border border-border rounded-xl overflow-hidden">
      <div className="p-3 border-b border-border">
        <button className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-coral-500/100 text-white rounded-lg hover:bg-coral-600 transition-colors text-sm">
          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  return (
    <div className="border rounded-xl overflow-hidden w-full max-w-xl mx-auto">
      <div className="flex items-center justify-between px-4 py-2 bg-muted/50 border-b border-border">
        ...
        <button className="text-sm text-coral-500 hover:text-coral-600">Reset</button>
      </div>
      <textarea
        placeholder="You are a helpful assistant..."
        rows={4}
  ...
  return (
    <div className="space-y-6 w-full max-w-md mx-auto">
      <div>
        ...
        <input type="range" min="0" max="2" step="0.1" defaultValue="0.7" className="w-full accent-coral-500" />
      ...
      <div>
        ...
        <input type="range" min="256" max="4096" step="256" defaultValue="2048" className="w-full accent-coral-500" />
      </div>
    </div>
  )
}
...
function StopButtonPreview() {
  return (
    <div className="flex gap-4">
      <button className="flex items-center gap-2 px-4 py-2 bg-red-500/100 text-white rounded-lg hover:bg-red-600 transition-colors">
    ...
    <div className="grid md:grid-cols-2 gap-4">
      <div className="border rounded-xl overflow-hidden">
        <div className="px-4 py-2 bg-purple-500/10 border-b border-border flex items-center gap-2">
          <span className="w-2 h-2 bg-purple-500/100 rounded-full"></span>
          <span className="font-medium text-purple-700">GPT-4</span>
        </div>
      ...
      <div className="border rounded-xl overflow-hidden">
        <div className="px-4 py-2 bg-coral-500/10 border-b border-border flex items-center gap-2">
          <span className="w-2 h-2 bg-coral-500/100 rounded-full"></span>
          <span className="font-medium text-coral-500">Claude</span>
        </div>
        <div className="p-4 text-sm text-muted-foreground">
    ...
    <div className="grid md:grid-cols-2 gap-4 max-w-2xl">
      <div className="aspect-square bg-muted rounded-xl flex items-center justify-center">
        <div className="text-center">
          <div className="w-12 h-12 border-4 border-coral-500 border-t border-border-transparent rounded-full animate-spin mx-auto"></div>
      ...
      <div className="relative group aspect-square bg-gradient-to-br from-purple-400 via-pink-500 to-coral-500 rounded-xl flex items-center justify-center text-white">
    ...
    <div className="space-y-4 w-full max-w-xl mx-auto">
      <div className="flex items-start gap-3 p-4 bg-red-500/10 border border-red-500/30 rounded-xl">
        ...
        <div>
          ...
          <p className="text-sm text-red-500 mt-1">Please wait a moment before trying again.</p>
          <button className="mt-2 text-sm text-red-600 hover:text-red-500 font-medium">
            Try again
          </button>
        </div>
      </div>
      <div className="flex items-start gap-3 p-4 bg-amber-500/10 border border-amber-500/30 rounded-xl">
        ...
        <div>
          <p className="font-medium text-amber-600">Token limit warning</p>
          <p className="text-sm text-amber-500 mt-1">You're approaching the context limit.</p>
        </div>
      </div>
    </div>
  )
